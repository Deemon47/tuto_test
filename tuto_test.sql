...
CREATE TABLE `books` (
  ...
  `description` text COLLATE utf8_unicode_ci COMMENT 'Краткое описание',
  `status` enum('public','hidden') COLLATE utf8_unicode_ci DEFAULT 'hidden',
  PRIMARY KEY (`id`),
  KEY `section_id_and_status` (`status`,`section_id`) USING BTREE,
  KEY `section_id` (`section_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='Учебники';
